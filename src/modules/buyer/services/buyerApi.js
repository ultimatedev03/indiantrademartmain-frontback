...
const getBuyerId = async () => {
  ...
  if (error || !buyer) {
    console.warn("Buyer profile not found for user:", user.id, error);
...
export const buyerApi = {
  ...
  createProposal: async (proposalData) => {
    ...
    try {
      const leadData = await insertLeadSafely(leadPayload);
  ...
  getSentLeads: async (buyerId) => {
    ...
    if (buyerId) {
      ...
      // If relation select fails, try plain
      try {
        const { data, error } = await supabase
          .from('leads')
          .select('*')
