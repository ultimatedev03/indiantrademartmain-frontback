        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              products.map(p => {
                const firstImage = p.product_images && p.product_images.length > 0 ? p.product_images[0].image_url : null;
                const categoryName = p.micro_category?.name || p.sub_category?.name || p.head_category?.name || 'Uncategorized';
                return (
                  <TableRow key={p.id}>
                    <TableCell>
