...
const ProductDetail = () => {
  ...
  const handleSubmitEnquiry = async () => {
    ...
    const safeBudget = budgetRaw ? budgetRaw.replace(/[\n\r\t]/g, ' ').slice(0, 80) : null;

    const qtyWithUnit = safeQty ? `${safeQty} ${enquiry.unit || 'pieces'}`.trim() : null;

    // ✅ Attach buyer contact details so vendor can see Buyer Name/Phone/Email in Lead details
    ...
    try {
      const { data: bp } = await supabase
        ...
        .eq('user_id', user.id)
        .maybeSingle();
      buyerProfile = bp || null;
    } catch (e) {
      console.warn('Failed to fetch buyer profile for enquiry lead:', e);
    }
    ...
    try {
      const plainProductDesc = data.description?.replace(/<[^>]*>/g, '') || '';
      ...
      const payload = {
        ...
        // buyer contact (so vendor can see who sent enquiry)
        ...
        // helpful context (won't hurt even if empty)
        product_desc_snapshot: plainProductDesc ? plainProductDesc.slice(0, 500) : null,
      };
      // ⚠️ Leads table schema may differ (some columns might not exist in your DB).
      // We'll try best insert, then fallback to a minimal safe payload.
      const tryInsert = async (dataToInsert) => supabase.from('leads').insert([dataToInsert]);
      ...
      if (res.error) {
        // Some DBs may not have these optional columns
        const { product_desc_snapshot, buyer_id, company_name, ...p1 } = payload;
        res = await tryInsert(p1);
      }
      if (res.error) {
        const { product_desc_snapshot, location, buyer_phone, ...p2 } = payload;
        res = await tryInsert(p2);
      }
      if (res.error) {
        res = await tryInsert(minimalPayload);
      }
