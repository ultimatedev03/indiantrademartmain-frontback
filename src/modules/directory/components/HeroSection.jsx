    ...
    <div className="relative bg-slate-900 pt-20 pb-24 lg:pt-32 lg:pb-40 overflow-hidden">
       ...
       <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6 tracking-tight">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-blue-400">Manufacturers & Suppliers</span>
          </h1>
          
          <p className="text-lg md:text-xl text-slate-300 mb-10 max-w-2xl mx-auto leading-relaxed">
            Discover verified business partners, source quality products, and grow your network with confidence on our secure platform.
          </p>
          ...
          <div className="max-w-4xl mx-auto">
            <form onSubmit={handleSearch} className="bg-white p-2 rounded-xl shadow-2xl flex flex-col md:flex-row gap-2">
               ...
               {/* Search Button */}
               <Button type="submit" className="h-14 px-8 bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg rounded-lg shadow-lg md:w-auto w-full transition-all duration-200">
                  Search
               </Button>
            </form>
          </div>
          ...
          <div className="mt-8 flex flex-wrap justify-center items-center gap-4 text-sm text-slate-400">
             ...
             <button onClick={() => setQuery('Industrial Machinery')} className="hover:text-blue-300 transition-colors px-3 py-1 bg-white/5 rounded-full border border-white/10">Industrial Machinery</button>
             <button onClick={() => setQuery('Textiles')} className="hover:text-blue-300 transition-colors px-3 py-1 bg-white/5 rounded-full border border-white/10">Textiles</button>
             <button onClick={() => setQuery('Chemicals')} className="hover:text-blue-300 transition-colors px-3 py-1 bg-white/5 rounded-full border border-white/10">Chemicals</button>
             <button onClick={() => setQuery('Electronics')} className="hover:text-blue-300 transition-colors px-3 py-1 bg-white/5 rounded-full border border-white/10">Electronics</button>
          </div>
       </div>
    </div>
  );
