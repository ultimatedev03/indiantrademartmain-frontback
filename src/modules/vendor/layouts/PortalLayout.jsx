...
const PortalLayout = () => {
  ...
  const loadVendorStatus = useCallback(async () => {
    ...
    // If user exists but fields missing, don't assume active=true.
    // safest: keep false until backend says true.
    const fallbackActive = user ? false : true;
    ...
    try {
      ...
      const activeVal = readBool(base, ['is_active', 'isActive'], fallbackActive);
